...
func GetAccount(ctx context.Context, accId int32) (*mercari.Account, error) {
	...
	acc := &mercari.Account{}
	if err := cache.GetHandler().Get(ctx, fmt.Sprintf(config.MercariAccountPrefix, accId), acc); err != nil {
		...
		go func() {
			if err := cache.GetHandler().Set(context.Background(), fmt.Sprintf(config.MercariAccountPrefix, accId), acc, time.Hour); err != nil {
