...
func (m *Mercari) GetBrands(ctx context.Context) (*GetBrandsResp, error) {
	getBrandFunc := func() (*GetBrandsResp, error) {
		...
		url := fmt.Sprintf("%s/v1/master/item_brands", m.OpenApiDomain)
	...
	result, err := backoff.Retry(ctx, getBrandFunc, retry.GetDefaultRetryOpts()...)
	if err != nil {
		pErr := &backoff.PermanentError{}
		if errors.As(err, &pErr) {
			hlog.CtxInfof(ctx, "get mercari item brands error: %v", err)
			berr := pErr.Unwrap()
			return nil, berr
		}
		hlog.CtxInfof(ctx, "get mercari item brands error: %v", err)
